...
//Similar to the provided trash acceleration function, but takes ship inputs into account.
void new_ship_acceleration(struct planet_stucture planets[], int total_planets,struct trash_ship ship[]){ 
    ...
    for (int n_ships = 0; n_ships < total_planets; n_ships ++){
        if (ship[n_ships].ID)
        {
            switch(ship[n_ships].input)
            {
                case 2:
                    ship[n_ships].acceleration.amplitude = 0.1;
                    ship[n_ships].acceleration.angle = -1.5708;
                    break;
                case 3:
                    ship[n_ships].acceleration.amplitude = 0.1;
                    ship[n_ships].acceleration.angle = 3.14159;
                    break;
                case 4:
                    ship[n_ships].acceleration.amplitude = 0.1;
                    ship[n_ships].acceleration.angle = 1.5708;
                ...
                case 5:
                    ship[n_ships].acceleration.amplitude = 0.1;
                    ship[n_ships].acceleration.angle = 0;
                    break;
                default:
                total_vector_force.amplitude = 0;
                total_vector_force.angle = 0;
...
//it then goes through each ship; if a ship is enabled, it checks its distance to all planets and all trashes
//if it's less than 10 to a planet, it unloads trash if it's a recycle planet or puts trash back in the universe otherwise
//if it's less than 10 to a trash, it picks it up if it has capacity
void check_collisions(struct planet_stucture planets[], int total_planets, struct trash_stucture trash[], int total_trash, int universe_dimensions,struct trash_ship ship[],int max_capacity) 
{
    ...
    {
        for(int h = 0;h < total_trash;h++) 
        {
            if (trash[h].status == 1)
            {
                if(sqrt(pow(trash[h].x - planets[i].x, 2) + pow(trash[h].y - planets[i].y, 2)) < 1 && are_all_ships_empty(ship, total_planets))
            ...
            for (int j = 0;j < total_planets;j++)
            {
                if(sqrt(pow(ship[i].x - planets[j].x, 2) + pow(ship[i].y - planets[j].y, 2)) < 10)
                {
                    if(!planets[j].isrecycle)
                    {
                        ...
                        planets[j].score += ship[i].capacity;
...
//All encompassing function that updates the physics of the universe
